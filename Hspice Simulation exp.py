...
def hspice_simulation(start_timing, end_timing, current_height, injection_node, output_node, input_file, injection_pulse):
	...
	while injection_timing <= stop_timing:
		...
		#在原 sp 文件中查找定义 current height 的命令行
		injection_current_height_line_index = find_index_2(temp_file, '.param current_height ')
		line = temp_file[injection_current_height_line_index[0]]
		new_line = ''.join([line.split('current_height = ')[0] + 'current_height = ' + current_height + 'e-6' + line.split('current_height = ')[1].split(' ', 1)[1] ])
		temp_file[injection_current_height_line_index[0]] = new_line	

		#在原 sp 文件中查找定义 injection timing 的命令行并 comment out 掉
		...
		#替换PWL描述部分
		#先把PWL波形的time和current都储存在list之中
		PWL_list = []
		for key, value in injection_pulse:
			PWL_list.append('%0.2f'%(float(key)+injection_timing)+'e-9 ')
			PWL_list.append(value + ' ')

		for line_number, line in enumerate(temp_file):
			if ('i_'+injection_node) in line and '*' not in line:
				#用来描述波形的部分夹在part1和part2之间
				part1 = line.split('PWL')[0]
				part2 = line.split('PWL')[1].split(' ')[-1]
				
				#用来替换的新的PWL描述
				new_line = []
				new_line.append(part1)
				new_line.append('PWL ')
				for part in PWL_list:
					new_line.append(part)
				new_line.append(part2)				
				temp_file[line_number] = ''.join(new_line)
		...
		#injection_timing = round(injection_timing + 0.3, 2)		

	#调用 input_file 文件夹中的每个 sp 文件进行一次 hspice 模拟, 输出文件的文件名与输入文件保持一致并保持在 output_file 文件夹中
	for sp_file in sorted(glob.glob('./input_file/' + combination_num + '_' + injection_node + '*.sp')):
...
#读取定义injection pulse波形的csv文件
injection_pulse_csv_file = csv.reader(open('./injection_pulse/Q=400_L=0.5.csv', 'rU'))
temp_dict = {}

for line in injection_pulse_csv_file:
	time = line[0].split(';')[0]
	current = '-'+line[0].split(';')[1]
	temp_dict[time] = str(float(current)/1000)

#根据时间点来进行排序
injection_pulse = sorted(temp_dict.items(), key = lambda list:list[0])


#'''
########## 进行 hspice simulation ##########
for data in input_data:
	hspice_simulation(data[0], data[1], data[2], data[3], data[4], input_file, injection_pulse)
#'''
